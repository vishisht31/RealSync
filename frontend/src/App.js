...
function App() {
    ...
    async (docId) => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/document/${docId}/versions`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        if (response.ok) {
          ...
          console.error(
            `Error fetching versions for ${docId}:`,
            response.statusText
          );
        }
      } catch (error) {
  ...
  const createDocument = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/document/createDocument`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        alert(
          `Error creating document: ${newDoc.error || response.statusText}`
        );
      }
    } catch (error) {
  ...
  const revertDocument = async (versionIndex) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/document/${currentDocumentId}/revert`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="document-editor-header">
        ...
        <div>
          <button
            ...
            className="document-controls button"
